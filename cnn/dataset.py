...
class Dataset():
    def inference_dataset(self):
        ...
        dataset = tf.data.TFRecordDataset(test_tfrecords, buffer_size=10240, num_parallel_reads=tf.data.experimental.AUTOTUNE, compression_type=Config.compression_type)
        return (dataset.map(lambda x: self.parse_image(x), num_parallel_calls=tf.data.experimental.AUTOTUNE), test_tfrecords[0])

    def training_dataset(self):
        train_tfrecords = glob.glob(Config.dataset_path)
        assert len(train_tfrecords) > 0
        dataset = tf.data.TFRecordDataset(train_tfrecords, buffer_size=10240, num_parallel_reads=tf.data.experimental.AUTOTUNE, compression_type=Config.compression_type)
        dataset = dataset.map(lambda x: self.parse_image(x), num_parallel_calls=tf.data.experimental.AUTOTUNE)
        dataset = dataset.take(Config.samples)
        if not Config.val_dataset_path:
            val_no = int(Config.val_dataset_split / 100. * Config.samples)
            val_dataset = dataset.take( val_no )
            dataset = dataset.skip( val_no )
        else:
            val_dataset = tf.data.TFRecordDataset(val_tfrecords, buffer_size=10240, num_parallel_reads=tf.data.experimental.AUTOTUNE, compression_type=Config.compression_type)
            val_dataset = val_dataset.map(lambda x: self.parse_image(x), num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    def parse_image(self, example_proto):
        ...
        x = tf.reshape(x, (Config.img_height, Config.img_width))  # (data['image/sizex'], data['image/sizey']))
        x = tf.expand_dims(x, -1)

        y_base = [data['image/particle/min_x'] / Config.img_width,
                  ...
                  tf.cast(data['image/particle/pid'], tf.float32) / Config.classes_no, # must be fixed
                  ]
