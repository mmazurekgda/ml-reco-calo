...
class YOLOCore():
    ...
    def refine_boxes(self, pred, anchors):
        ...
        box_xy = (box_xy + tf.cast(grid, tf.float32)) / tf.cast([grid_size_x, grid_size_y], tf.float32)
    ...
    def yolo_loss(self, anchors):
        def yolo_loss(y_true, y_pred):
            ...
            pred_box, pred_energy, pred_obj, pred_class, pred_xywh = self.refine_boxes(y_pred, anchors)
            ...
            true_box, true_energy, true_obj, true_class_idx = tf.split(y_true, (4, 1, 1, 1), axis=-1)
            ...
            energy_loss_scale = 1. #((true_energy[..., 0] * Config.z_score_std + Config.z_score_mean) / 4000.0 / 7.66 + 1.0) ** 2.
            ...
            true_xy = true_xy * tf.cast([grid_size_x, grid_size_y], tf.float32) - tf.cast(grid, tf.float32)
            ...
            xy_loss = obj_mask * box_loss_scale * energy_loss_scale * tf.reduce_sum(tf.square(true_xy - pred_xy), axis=-1)
            wh_loss = obj_mask * box_loss_scale * energy_loss_scale *  tf.reduce_sum(tf.square(true_wh - pred_wh), axis=-1)
            energy_loss = obj_mask * box_loss_scale * energy_loss_scale * tf.reduce_sum(tf.square(true_energy - pred_energy), axis=-1)
            ...
            return 0.25 * xy_loss  + 0.25 * obj_loss + 0.25 * wh_loss # + .25 * energy_loss  #+ 0.125 * class_loss
        return yolo_loss
    ...
    def transform_targets_for_output(self, y_true, grid_size_x, grid_size_y, anchor_idxs):
        ...
        y_true_out_en = tf.zeros((grid_size_y, grid_size_x, tf.shape(anchor_idxs)[0], 1))
        ...
        ys_selected_anchors = [ tf.equal(int(anchor_idxs[idx]), tf.cast(y_true[..., 6], tf.int32)) for idx in range(len(anchor_idxs)) ]
        ys_selected_anchor = ys_selected_anchors[0]
        if tf.shape(anchor_idxs)[0] > 1:
            for anchor_idx in anchor_idxs[1:]:
                ys_selected_anchor = tf.logical_or(ys_selected_anchor, tf.equal(int(anchor_idx), tf.cast(y_true[..., 6], tf.int32)))
        y_anchored = tf.gather(y_true, tf.where(tf.squeeze(ys_selected_anchor, axis=0)), axis=1)
        ...
        updates_l  = tf.cond(tf.size(updates_l) == 4, lambda: tf.reshape(updates_l, (1,  4)), lambda: updates_l)
        updates_r  = tf.squeeze(tf.concat([obj,  y_anchored[..., 5:6]], axis=-1))
        updates_r  = tf.cond(tf.size(updates_r) == 2, lambda: tf.reshape(updates_r, (1,  2)), lambda: updates_r)
        updates_en = tf.squeeze(y_anchored[..., 4:5], axis=[0, 2])
        #tf.print(tf.shape(updates_en))
        #tf.print(updates_en)
        updates_en  = tf.cond(tf.size(updates_en) == 1, lambda: tf.reshape(updates_en, (1,  1)), lambda: updates_en)
        # prepare indices that will be use to put values in the yolo grid
        indices = tf.squeeze(tf.concat([grid_y, grid_x, tf.cast(y_anchored[..., 6:7], tf.int32) - min_anchor_idx], axis=-1))
        indices = tf.cond(tf.size(indices) == 3, lambda: tf.reshape(indices, (1,  3)), lambda: indices)
        ...
        y_true_out = tf.concat([y_true_out_l, y_true_out_en, y_true_out_r,], axis=-1)
        return y_true_out
    ...
    def transform_targets(self, y_train):
        ...
        intersection = tf.minimum(box_wh[..., 0], anchors_tf[..., 0]) * tf.minimum(box_wh[..., 1], anchors_tf[..., 1])
