...
def clusterize_barycenter(
    ...
    hits["Cell_X_Central"] = (
        hits["Cell_X"] * hits["Active_Energy"] / hits["Active_Energy_Sum"]
    )
    hits["Cell_Y_Central"] = (
        hits["Cell_Y"] * hits["Active_Energy"] / hits["Active_Energy_Sum"]
    )
    hits_for_barycenter = (
        hits[
            [
    ...
    hits = hits.merge(
        hits_for_barycenter[
            ["Event_ID", "Particle_Index", "Cell_X_Central", "Cell_Y_Central"]
        ],
...
def clusterize(
    ...
    hits_grouped = hits_df[cols_to_select].groupby(by=cols_to_groupby, as_index=False)
    ...
    hits = hits_df.merge(central_cells, on=cols_to_groupby, suffixes=("", "_Central"))
    hits = hits.merge(energy_sum, on=cols_to_groupby, suffixes=("", "_Sum"))
    ...
    hits["Distance"] = abs(hits["Cell_X"] - hits["Cell_X_Central"]) + abs(
        hits["Cell_Y"] - hits["Cell_Y_Central"]
    )
    hits["IsMinCluster"] = (
        hits["Distance"] < min_cell_size_no * hits["Cell_Size_Central"]
    )
    hits["IsMaxCluster"] = (
        hits["Distance"] < max_cell_size_no * hits["Cell_Size_Central"]
    )
    # sort the hits, distance ascending, energy descending
    hits = hits.sort_values(
        ["Event_ID", "Particle_Index", "Distance", "Active_Energy"],
        ascending=[1, 1, 1, 0],
    )
    hits["Energy_Cum"] = (
        hits[["Event_ID", "Particle_Index", "Active_Energy"]]
        .groupby(["Event_ID", "Particle_Index"])
        .cumsum()
    )
    hits["IsCumEnergyLessMinEnergyRatio"] = (
        hits["Energy_Cum"] / hits["Active_Energy_Sum"] < min_energy_ratio
    )
    ...
    hits = hits[
        (hits["IsCumEnergyLessMinEnergyRatio"] | hits["IsMinCluster"])
        & hits["IsMaxCluster"]
    ]
    hits.drop(columns=["Active_Energy_Sum"], inplace=True)

    hits_energy = hits[cols_to_select].groupby(by=cols_to_groupby, as_index=False).sum()

    hits_energy.columns = [
        col + (new_suffix if col not in cols_to_groupby else "")
        for col in hits_energy.columns
    ]
    ...
    hits_central = (
        hits[
            cols_to_groupby + ["Cell_X_Central", "Cell_Y_Central", "Cell_Size_Central"]
        ]
        .groupby(by=cols_to_groupby, as_index=False)
        .max()
    )

    hits_central.columns = [
        col + (new_suffix if col not in cols_to_groupby else "")
        for col in hits_central.columns
    ]
    ...
    if compute_barycenter or use_barycenter:
        hits["Position_X"] = hits["Cell_X"] + hits["Cell_Size"] / 2.0
        hits["Position_Y"] = hits["Cell_Y"] + hits["Cell_Size"] / 2.0
        ...
        hits_barycenter_stdx = hits_grouped_std.apply(
            weighted_std(
        ...
        hits_barycenter_meanx = hits_grouped_std.apply(
            weighted_mean(
                values_col="Position_X",
                weights_col="Active_Energy",
        ...
        hits_barycenter_stdy = hits_grouped_std.apply(
            weighted_std(
        ...
        hits_barycenter_meany = hits_grouped_std.apply(
            weighted_mean(
                values_col="Position_Y",
                weights_col="Active_Energy",
        ...
        hits_barycenter.columns = [
            col + (new_suffix if col not in cols_to_groupby else "")
            for col in hits_barycenter.columns
        ]
    ...
    if use_barycenter:
        ...
        std_from_cell = (
            min_barycenter_cell_size_no
            * particles_df["Cell_Size_Central" + new_suffix]
            / 2.0
        )

        particles_df["xstddev"] = (
            stddev_no * particles_df["Barycenter_X_stddev" + new_suffix]
        )
        x_smaller_or_null = particles_df["xstddev"].isna() | (
            particles_df["xstddev"] < std_from_cell
        )
        x_smaller_or_null = x_smaller_or_null | particles_df["xstddev"].isin(
            [np.nan, np.inf, -np.inf]
        )
        particles_df.loc[x_smaller_or_null, "xstddev"] = std_from_cell

        particles_df["ystddev"] = (
            stddev_no * particles_df["Barycenter_Y_stddev" + new_suffix]
        )
        y_smaller_or_null = particles_df["ystddev"].isna() | (
            particles_df["ystddev"] < std_from_cell
        )
        y_smaller_or_null = y_smaller_or_null | particles_df["ystddev"].isin(
            [np.nan, np.inf, -np.inf]
        )
        particles_df.loc[y_smaller_or_null, "ystddev"] = std_from_cell

        particles_df["Cell_X_Min" + new_suffix] = (
            particles_df["Barycenter_X" + new_suffix] - particles_df["xstddev"]
        )
        particles_df["Cell_X_Max" + new_suffix] = (
            particles_df["Barycenter_X" + new_suffix] + particles_df["xstddev"]
        )
        particles_df["Cell_Y_Min" + new_suffix] = (
            particles_df["Barycenter_Y" + new_suffix] - particles_df["ystddev"]
        )
        particles_df["Cell_Y_Max" + new_suffix] = (
            particles_df["Barycenter_Y" + new_suffix] + particles_df["ystddev"]
        )

        particles_df.loc[
            particles_df["Cell_X_Min" + new_suffix] < img_x_min,
            "Cell_X_Min" + new_suffix,
        ] = img_x_min
        particles_df.loc[
            particles_df["Cell_X_Max" + new_suffix] > img_x_max,
            "Cell_X_Max" + new_suffix,
        ] = img_x_max
        particles_df.loc[
            particles_df["Cell_Y_Min" + new_suffix] < img_y_min,
            "Cell_Y_Min" + new_suffix,
        ] = img_y_min
        particles_df.loc[
            particles_df["Cell_Y_Max" + new_suffix] > img_y_max,
            "Cell_Y_Max" + new_suffix,
        ] = img_y_max
        ...
        hits = hits.groupby(["Event_ID", "Particle_Index"])[
            ["Cell_X", "Cell_Y", "Cell_X_Extended", "Cell_Y_Extended"]
        ].agg(["max", "min"])
        hits = hits.drop(
            columns=[
                ("Cell_X", "max"),
        ...
        hits.columns = [
            ("_".join([col.title() for col in cols])).strip().replace("_Extended", "")
            for cols in hits.columns.values
        ]
        hits.columns = [
            col + (new_suffix if col not in cols_to_groupby else "")
